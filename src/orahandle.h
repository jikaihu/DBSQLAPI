...
#ifndef H_ORAHANDLE_H
#define H_ORAHANDLE_H
...
typedef sword (*OCIDateTimeGetTime_t)(dvoid *hndl,
				OCIError *err, OCIDateTime *datetime,
				ub1 *hour, ub1 *min, ub1 *sec, ub4 *fsec);
typedef sword (*OCINumberToInt_t)(OCIError *err, CONST OCINumber *number,
                                     uword rsl_length, uword rsl_flag, dvoid *rsl    );
// API declarations
class oraHandle:public dbHandle
{
public:
    oraHandle();
    virtual ~oraHandle();
    ...
    OCINumberToInt_t OCINumberToInt;
};
